...
const handleMenuToggle = () => {
    ...
    return(
        <nav>
            ...
            {/* Mobile */}
            <div className={`${isMenuOpen ? 'fixed' : 'relative'} bg-black top-0 left-0 z-[100] block md:hidden`}>
                <button onClick={handleMenuToggle} className="text-white">
                        <img src="/Menu.svg" 
                        className="w-[50px] h-[50px] m-[10px]"  alt="menu"/>
                    </button>
                <div className={`${isMenuOpen ? 'w-screen h-screen ' : 'bg-none w-0 h-0'}  transition-all duration-250`}>
